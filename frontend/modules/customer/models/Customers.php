...
class Customers extends \yii\db\ActiveRecord {
    ...
    function Getexpirepromise() {
        $userid = \Yii::$app->user->identity->id;
        $customer = \app\modules\customer\models\Customers::findOne(['user_id' => $userid]);
        $customerId = $customer['id'];
        $sql = "select * from promise where customerid = '$customerId' and status = '2'";
        $rePromise = Yii::$app->db->createCommand($sql)->queryOne();
        if ($rePromise) {
            $dStart = date("Y-m") . "-01";
            $dEtart = substr($rePromise['promisedateend'], 0, 7) . "-30";
