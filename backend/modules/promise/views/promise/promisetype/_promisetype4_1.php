...
<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>ข้อ 2  </strong>“ผู้รับจ้าง” ตามข้อ 1 สัญญาว่าจะเริ่มนับตั้งแต่  วันที่ <?php echo $Config->thaidate($model['promisedatebegin']);?>  ถึงวันที่ <?php echo $Config->thaidate($model['promisedateend']);?>  รวมระยะเวลา 12  เดือน  ถ้าผู้รับจ้าง<br>มิได้ลงมือทำงานภายในกำหนดเวลา  หรือมีเหตุให้เชื่อได้ว่าผู้รับจ้างไม่สามารถทำงานให้แล้วเสร็จภายในกำหนดเวลา  หรือล่าช้าเกินกว่ากำหนด<br>เวลา  หรือผู้รับจ้างทำผิดข้อตกลงข้อใดข้อหนึ่ง ผู้ว่าจ้างมีสิทธิที่จะบอกเลิกจ้างตามบันทึกนี้ได้ 
<?php if($model['payment']==0 ){?>
    <br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>ข้อ 3  </strong> “ผู้ว่าจ้าง”  ตกลงจ่าย  และผู้รับจ้างตกลงรับเงินค่าจ้างเป็นรายครั้ง    รวมระยะเวลา  12     เดือน      โดยกำหนด ค่าจ้างต่อครั้ง ต้องน้ำหนัก<br>ไม่เกิน <?php echo $model['garbageweight']?> กิโลกรัม  โดยปริมาณน้ำหนักขยะส่วนที่เกิน <?php echo $model['garbageweight']?> กิโลกรัมขึ้นไป ทางบริษัทฯ จะคิดค่าขยะส่วนเกินที่เพิ่มขึ้นมากิโลกรัมละ <?php echo $model['fine']?> บาท  ขยะที่<br>“ผู้รับจ้าง” เก็บขนย้ายไปทำลายในแต่ละเดือน คิดค่าจ้างเหมาในอัตราครั้งละ <?php echo number_format($model['rate']);?>  บาท (<?php echo $Config->Convert($model['rate'])?>)  จัดเก็บ <?php echo $model['levy'];?> ครั้งต่อ เดือน เป็นค่าจ้าง<br>รวมทั้งสิ้นต่อปี <?php echo number_format($model['total']);?>  บาท (<?php echo $Config->Convert($model['total'])?>)  โดยกำหนดจ่ายภายใน  30 วัน นับตั้งแต่วันที่ “ผู้ว่าจ้าง” หรือตัวแทนของ “ผู้ว่าจ้าง” ได้ทำการตรวจรับถูกต้องเรียบร้อยแล้ว
<?php }else if($model['payment'] == 1){?>
    <br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>ข้อ 3 </strong> “ผู้ว่าจ้าง”  ตกลงจ่าย  และผู้รับจ้างตกลงรับเงินค่าจ้างเป็นรายปี  ระยะเวลา  1  ปี โดยคิดค่าจ้างตามปริมาณน้ำหนักขยะไม่เกิน <?php echo $model['garbageweight']?> กิโลกรัมต่อครั้ง  ในอัตราครั้งละ  <?php echo number_format($model['rate']);?> บาท (<?php echo $Config->Convert($model['rate'])?>) ส่วนปริมาณน้ำหนักขยะส่วนที่เกิน <?php echo $model['garbageweight']?> กิโลกรัมขึ้นไป จะคิดค่าจัดเก็บขยะเพิ่ม ในอัตรากิโลกรัมละ <?php echo $model['fine']?> บาท(<?php echo $Config->Convert($model['fine'])?>) “ผู้รับจ้าง” เก็บขนย้ายไปทำลายในแต่ละเดือน โดยเข้าจัดเก็บเดือนละ <?php echo $model['levy'];?> ครั้ง รวม 12 ครั้งต่อปี แต่เนื่องด้วยผู้ว่าจ้างเลือกจ่ายชำระเงินเป็นเป็นรายปี จึงได้รับส่วนลด <?php echo  number_format($model['distcountbath'])?> บาท (<?php echo $Config->Convert($model['distcountbath'])?>) คิดเป็นค่าจ้างรวมทั้งปี <?php echo  number_format($model['total'])?> บาท (<?php echo $Config->Convert($model['total'])?>)  โดยกำหนดจ่ายชำระภายใน 30 วัน นับตั้งแต่วันที่ “ผู้ว่าจ้าง” หรือตัวแทนของ “ผู้ว่าจ้าง” ได้ทำการตรวจรับ ถูกต้องเรียบร้อยแล้ว
<?php }?>
<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>ข้อ 4 หน้าที่รับผิดชอบของ “ผู้รับจ้าง” </strong>
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.1  “ผู้รับจ้าง” จะต้องทำการเก็บขนย้ายและกำจัดขยะมูลฝอยติดเชื้อโดยใช้พนักงานที่มีความรู้ผ่านการอบรมการกำจัดขยะติดเชื้อ<br>และขนย้าย เพื่อป้องกันอันตรายจาการเก็บขยะ และปฏิบัติตามหลักเกณฑ์ที่กฎหมายและระเบียบได้กำหนดไว้อย่างเคร่งครัด เพื่อให้เป็นไปตาม<br>กฎกระทรวงสาธารณสุขว่าด้วยการกำจัดขยะมูลฝอยติดเชื้อ พ.ศ. 2545
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4.2  “ผู้รับจ้าง” จะต้องเป็นผู้มีอาชีพรับจ้างขนและกำจัดขยะมูลฝอยติดเชื้อที่มีสถานที่ผู้ได้รับใบอนุญาตกำจัดขยะมูลฝอยติดเชื้อ ซึ่งเป็นนิติบุคคลผู้มีอาชีพรับจ้างกำจัดขยะมูลฝอยติดเชื้อ ไว้รองรับการกำจัดขยะมูลฝอยติดเชื้อ โดยวิธีเผาในเตาเผาขยะมูลฝอยติดเชื้อ ตลอดระยะเวลาตามสัญญานี้ด้วย
...
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5.2  “ผู้ว่าจ้าง” จะต้องจัดเจ้าหน้าที่ผู้รับผิดชอบขยะมูลฝอยติดเชื้อไว้ ประสานงานและร่วมมือรับทราบการบันทึกน้ำหนักขยะ ตลอดจนการลงลายมือชื่อร่วมไว้ในเอกสารดังกล่าว ตามวันและเวลาที่ทั้งสองฝ่ายได้ตกลงกำหนดไว้ด้วย
		...
		<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5.4 หาก”ผู้ว่าจ้าง”เปิดทำการไม่ตรงเวลาทำการของ”ผู้รับจ้าง” หรือมีเหตุให้ต้องหยุดทำการในวันและเวลาที่ตกลงกันไว้ “ผู้ว่าจ้าง” จะต้องวางขยะ ไว้ในจุดที่ “ผู้รับจ้าง”สามารถเก็บขนได้ หาก “ผู้ว่าจ้าง”ไม่วางขยะตามกำหนดวันและเวลาที่ตกลงกันไว้”ผู้ว่าจ้าง”ยินยอมจ่ายค่าบริการให้ “ผู้รับจ้าง”
...
<br><br>
<strong>หมายเหตุ</strong> : ทาง “ผู้รับจ้าง” จะเข้าดำเนินการจัดเก็บขยะมูลฝอยติดเชื้อให้กับ “ผู้ว่าจ้าง”  หลังจากทำสัญญาแล้วประมาณ  1  เดือน
